// GPU information module for zfetch.
...
fn looks_like_product_name(name: &str) -> bool {
    name.contains("RX ")
        || name.contains("RTX ")
        || name.contains("GTX ")
...
fn gpu_fresh() -> GpuInfo {
    ...
    if let Some(ref name) = info.discrete.clone() {
        if looks_like_product_name(name) {
            info.discrete = Some(clean_gpu_name(name));
        }
    }

    // Only refine if sysfs gave a raw chip name (e.g. "AMD Navi 48") rather than
    // a full product name (e.g. "AMD RX 9070 XT") — skip subprocess if already good
    if info.discrete.is_some()
        && !looks_like_product_name(info.discrete.as_deref().unwrap_or(""))
    {
        if let Some(specific_name) = gpu_from_vulkaninfo(false) {
            if is_clearly_discrete_gpu(&specific_name) {
...
fn igpu_from_cpuinfo() -> Option<String> {
    ...
    let gpu_start = memmem::find(name_bytes, b" with ")
        .or_else(|| memmem::find(name_bytes, b" w/ "))?;
...
fn gpu_from_sysfs_multi() -> Option<GpuInfo> {
    ...
    // Resolve AMD iGPU name: prefer cpuinfo name (more specific) over pci.ids name
    let resolve_igpu_name = |card: &CardInfo| -> String {
        let name = if card.vendor_id == "1002" {
            cpu_igpu_name.clone().unwrap_or_else(|| card.full_name.clone())
        } else {
    ...
    for card in &cards {
        ...
        if is_discrete {
            ...
            if info.integrated.is_none() {
                info.integrated = Some(resolve_igpu_name(card));
            }
            ...
            if is_integrated && info.integrated.is_none() {
                info.integrated = Some(resolve_igpu_name(card));
            } else if !is_integrated && info.discrete.is_none() {
                info.discrete = Some(card.full_name.clone());
            }
        }
    }
...
fn gpu_from_lspci(allow_igpu: bool) -> Option<String> {
    ...
    while search_pos < stdout.len() {
        ...
        if fields.len() >= 3 {
            ...
            let is_integrated = is_integrated_gpu(device, "", None);
